                ...
                <div class="card-body">
                    <form action="<?= site_url('outcomes/' . $outcome['id']) ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="3"><?= old('description', $outcome['description']) ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="link" class="form-label">Link (URL)</label>
                            <input type="url" class="form-control" id="link" name="link" value="<?= old('link', $outcome['link']) ?>" placeholder="https://example.com">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">Supported formats: PDF, DOC, DOCX, TXT, JPG, PNG. Leave empty to keep current file.</div>
                            <?php if ($outcome['file_path']): ?>
                                <div class="mt-2">
                                    <small class="text-muted">Current file: <?= esc($outcome['file_path']) ?></small>
                                </div>
                            <?php endif; ?>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="<?= site_url('outcomes/' . $outcome['id']) ?>" class="btn btn-secondary">Cancel</a>
